...
NO_FINDINGS = "No findings were gathered. Provide a brief report acknowledging this."
...
def _format_findings(web_findings: list[str], doc_findings: list[str]) -> str:
    ...
    return "\n\n".join(sections) if sections else NO_FINDINGS
...
def synthesizer_node(state: ResearchState) -> dict:
    ...
    response = llm.invoke([
        ...
        HumanMessage(content=(
            ...
            f"Research plan: {state.get('plan') or 'N/A'}\n\n"
            f"{findings_text}\n\n"
    ...
    return {
        "final_report": message_text(response),
        "current_agent": "done",
