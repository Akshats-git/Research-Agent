...
Ask a research question and get back a sourced report — then keep talking to it. Four AI agents split the research work: one plans, one searches the web, one reads documents you upload, and one writes the report. Once a report exists you can ask follow-up questions or request edits, and every exchange is saved in a chat sidebar like the tools you already use.

Built with LangGraph, FastAPI, and Next.js.

## What it does

- **Runs a real research pipeline.** A new question spins up four agents that plan, search, analyse documents, and synthesise a structured report — streamed to the browser step by step.
- **Understands what you actually want.** Every message is classified first, so replying "ok" or "which sources did you use?" gets a conversational answer instead of re-running the whole pipeline. Ask for changes ("make it shorter", "add a section on salaries") and it revises the existing report in place.
- **Keeps your history.** Chats live in a sidebar with pin, rename, archive, and delete — persisted locally so a reload never loses your work.
...
4. The synthesizer turns the findings into a report — executive summary, key findings, conflicts between sources, knowledge gaps, and recommendations.

The orchestrator runs at most three times per query, which keeps the agents from looping forever. If no documents are uploaded, it never routes to the document analyst.

## The conversational layer

Not every message is a research request, so the backend classifies each turn before doing any work. The classifier sees the chat history and whether a report already exists, then picks one of three actions:

| Action | When | What happens |
|--------|------|--------------|
| **research** | A genuinely new topic or an explicit request for a report | Runs the full four-agent pipeline |
| **answer** | A follow-up question, a clarification, or an acknowledgement ("ok", "thanks") | A single conversational reply — no pipeline, no new report |
...
This is what stops the classic failure where every reply, even "ok", kicks off a brand-new report.
...
│   ├── conversation.py           Intent router + answer/revise handlers
...
└── frontend/src/app/components/
    ...
    ├── store.ts                  Client chat store, persisted to localStorage
...
The backend exposes one endpoint, `POST /api/chat`, and streams progress back as [Server-Sent Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). Each turn emits an `intent` event with the router's decision, then either a run of `agent_update` events (research), a `message` event (answer), or a `report` event (revise), and finally `complete`. The frontend reads the stream and updates the active chat in place.
